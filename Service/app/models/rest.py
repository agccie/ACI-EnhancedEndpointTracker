...
def register(api):
    ...
    for classname in sorted(registered_classes):
        ...
        for attr in c._attributes:
            if c._attributes[attr]["key"]:
                ...
                if key_type in ["string","int","float","path","any","uuid"]:
                    ...
                    cls.logger.error("invalid type for key: [%s, %s, %s]" % (
                        cls.__name__, attr, _type))
        ...
        cls._swagger = {}
        path = "/%s" % "/".join(cls._classname.split("."))
        key_path = "%s/%s" % (path, key_string) if len(key_string)>0 else path
        key_swag_path = re.sub("<[a-z]+:([^>]+)>",r"{\1}", key_path)

        # add create path
        if c._access["create"]:
            endpoint = "%s_create" % cls.__name__.lower()
            api.add_url_rule(path, endpoint , c.api_create, methods=["POST"])
            cls.logger.debug("registered create path: POST %s" % path)
            if path not in cls._swagger: cls._swagger[path] = {}
            swagger_create(cls, path)
        ...
        if c._access["read"]:
            if key_path is not None:
                endpoint = "%s_read" % cls.__name__.lower()
                api.add_url_rule(key_path, endpoint, c.api_read,methods=["GET"])
                cls.logger.debug("registered read path: GET %s" % key_path)
                swagger_read(cls, key_swag_path, bulk=False)

            if c._access["bulk_read"] and key_path!=path:
                endpoint = "%s_bulk_read" % cls.__name__.lower()
                api.add_url_rule(path,endpoint, c.api_read, methods=["GET"])
                cls.logger.debug("registered bulk read path: GET %s" % path)
                swagger_read(cls, path, bulk=True)

        # add update paths
        if c._access["update"]:
            if key_path is not None:
                endpoint = "%s_update" % cls.__name__.lower()
                api.add_url_rule(key_path,endpoint,c.api_update,
                    methods=["PATCH","PUT"])
                cls.logger.debug("registered update path: PATCH,PUT %s"%(
                    key_path))
                swagger_update(cls, key_swag_path, bulk=False)

            if c._access["bulk_update"] and key_path!=path:
                endpoint = "%s_bulk_update" % cls.__name__.lower()
                api.add_url_rule(path,endpoint,c.api_update,
                    methods=["PATCH","PUT"])
                cls.logger.debug("registered bulk update path: PATCH,PUT %s"%(
                    path))
                swagger_update(cls, path, bulk=True)

        # add delete paths
        if c._access["delete"]:
            if key_path is not None:
                endpoint = "%s_delete" % cls.__name__.lower()
                api.add_url_rule(key_path,endpoint,c.api_delete,
                    methods=["DELETE"])
                cls.logger.debug("registered delete path: DELETE %s"%key_path)
                swagger_delete(cls, key_swag_path, bulk=False)

            if c._access["bulk_delete"] and key_path!=path:
                endpoint = "%s_bulk_delete" % cls.__name__.lower()
                api.add_url_rule(path,endpoint,c.api_delete,methods=["DELETE"])
                cls.logger.debug("registered bulk delete path: DELETE %s"%path)
                swagger_delete(cls, path, bulk=True)

        # handle custom routes
        for r in c._access["routes"]:
            if "function" not in r or not callable(r["function"]):
                cls.logger.warn("%s skipping invalid route: %s, bad function"%(
                    cls._classname, r))
                continue
            if "path" not in r or len(path)==0:
                cls.logger.warn("%s skipping invalid route: %s, bad path"%(
                    cls._classname, r))
                continue
            ...
            if "methods" in r and isinstance(r["methods"], list):
                for m in r["methods"]:
                    if m not in ["GET","DELETE","POST","PATCH","PUT"]:
                        cls.logger.warn("%s invalid method: (%s) %s" % (
                            cls._classname, r, m))
                        continue
                    if m not in methods: methods.append(m)
                if len(methods)==0:
                    cls.logger.warn("%s no valid methods %s" % (
                        cls._classname, r))
                    continue
            else: methods=["GET"]
            endpoint="%s_%s" % (cls.__name__.lower(), r["function"].__name__)
            api.add_url_rule(rpath, endpoint,r["function"],methods=methods)
            cls.logger.debug("registered custom path: %s %s"%(methods, rpath))
            ...
            swagger_generic_path(cls,swag_rpath,methods[0],summary)
...
class Rest(object):
    """ generic REST object providing common rest functionality.  All other 
        ...
        read/update/delete operations. Additionally, kwargs may include the 
        following:
...

        {
            ...
            "routes":       list of custom (non-CRUD) routes to provide to API.
                    ...
                    "path":     str, string url path with corresponding args
                                ...
                                classname. For example, Users class located at
                                /api/users can add a 'login' path which will be
                                pushed to /api/users/login

                    "keyed_url" bool, if true then path will include object
                                keys before appending provided path. For 
                                example, Users class located at /api/users with
                                key "<string:username>" could have a 'login'
                                path requiring key in the url resulting in the
                                path: /api/users/<string:username>/login
    ...
    ACCESS_DEF = {
        "expose_id": False,
        "read_role": Roles.USER,
        "write_role": Roles.FULL_ADMIN,
        "execute_role": Roles.FULL_ADMIN,
        "create": True,
        "read": True,
        "update": True,
        "delete": True,
